...
def fetch_and_store():
    ...
    # API 파라미터 설정
    params = {
        "serviceKey": "Qi2b1kjmhOrxDGWxXvWoA8A9PFL0cIRXT/q5fEDhx51AE3S0IqmYBhu8f0hoX5W3ocdSRxFYmWh6OD4loS4G1g==",  # 본인의 인증키로 교체하세요
        "pageNo": "1",
        "numOfRows": "100",
        "dataType": "JSON",
        "base_date": base_date,
    ...
    try:
        ...
        print(f"API 응답 내용 요약: {data}")
        print(f"가져온 데이터 수: {len(items)}")


    except Exception as e:
        import traceback
        print(f"[{datetime.datetime.now()}] 오류 발생:")
        traceback.print_exc()
